...
class EarleyChart:
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        if nonterminal in self.cols[position]._predicted:
            return
        
        self.cols[position]._predicted.add(nonterminal)
        
        for rule in self.grammar.expansions(nonterminal):
            new_item = Item(rule, dot_position=0, start_position=position, weight=rule.weight, backpointer=None)
    ...
    def _scan(self, item: Item, position: int) -> None:
        ...
        if position < len(self.tokens) and self.tokens[position] == item.next_symbol():
            # Create backpointer: (previous_item, scanned_word)
            scanned_word = self.tokens[position]
            backpointer = (item, scanned_word)
        
            new_item = item.with_dot_advanced()
        # Set weight and backpointer
            new_item = Item(new_item.rule, new_item.dot_position, new_item.start_position, 
                            new_item.weight, backpointer)
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].get_customers(item.rule.lhs):
            backpointer = (customer, item)
        
            new_item = customer.with_dot_advanced()
            total_weight = customer.weight + item.weight
            new_item = Item(new_item.rule, new_item.dot_position, 
                        new_item.start_position, total_weight, backpointer)
            self.cols[position].push(new_item)
            log.debug(f"\tAttached to get: {new_item} in column {position}")
            self.profile["ATTACH"] += 1

    def best_weight(self) -> Optional[float]:
        """Return weight of the best complete parse, or None if rejected."""
        best = math.inf
        for item in self.cols[-1].all():
            if (item.rule.lhs == self.grammar.start_symbol and item.next_symbol() is None and item.start_position == 0):
                best = min(best, item.weight)
        return None if best == math.inf else best
    
    def get_parse_tree(self, item: Item) -> str:
        """Recursively reconstruct the parse tree from an item's backpointers.
        Returns the parse tree for all symbols matched so far (everything before the dot).
        """
    
    # Base case: item has no backpointer (predicted item at dot position 0)
        if item.backpointer is None:
            return ""
    
        backpointer = item.backpointer
    
    # Case 1: Scan operation - backpointer = (previous_item, scanned_word)
        if isinstance(backpointer[1], str):
            previous_item, word = backpointer
        
        # Recursively get the tree for everything before this word
            previous_tree = self.get_parse_tree(previous_item) if previous_item.dot_position > 0 else ""
        
        # Append the scanned word
            if previous_tree:
                return f"{previous_tree} {word}"
            else:
                return word
    
    # Case 2: Attach operation - backpointer = (customer_item, completed_item)
        else:
            customer_item, completed_item = backpointer
        
        # The completed_item is a full constituent - recursively get its children
            completed_children = self.get_parse_tree(completed_item)
        
        # Wrap the completed constituent with its nonterminal label
            completed_tree = f"({completed_item.rule.lhs} {completed_children})"
        
        # Recursively get the tree for what the customer had matched before
            customer_tree = self.get_parse_tree(customer_item) if customer_item.dot_position > 0 else ""
        
        # Combine customer's tree with the completed constituent
            if customer_tree:
                return f"{customer_tree} {completed_tree}"
            else:
                return completed_tree
            
    def get_best_parse(self) -> Optional[str]:
        """Return the best parse tree for the sentence, or None if rejected."""
        best_weight = math.inf
        best_item = None
        
        # Find the best complete parse in the last column
        for item in self.cols[-1].all():
        ...
        if best_item is None:
            return None
        children = self.get_parse_tree(best_item)
        return f"({best_item.rule.lhs} {children})"
...
class Agenda:
    ...
    def __init__(self) -> None:
        ...
        self._customers: Dict[Optional[str], List[Item]] = {}
    ...
    def push(self, item: Item) -> None:
        ...
        if not isinstance(item, Item):
            existing_idx = self._index.get(item)
        ...
        if existing_idx is None:
        # Brand new item - add it to the agenda
            self._items.append(item)
            self._index[key] = len(self._items) - 1
            
            next_sym = item.next_symbol()
            if next_sym not in self._customers:
                self._customers[next_sym] = []
            self._customers[next_sym].append(item)
            
        else:
        # Found a duplicate item
            existing = self._items[existing_idx]
        
        # Only update if both have weights and new one is better
            if hasattr(item, "weight") and hasattr(existing, "weight"):
                if item.weight < existing.weight:
                # New item has better (lower) weight
                # Update the item in place
                    self._items[existing_idx] = item
                
                    next_sym = item.next_symbol()
                    if next_sym in self._customers:
                
                        for i, cust in enumerate(self._customers[next_sym]):
                            if (cust.rule == item.rule and 
                                cust.dot_position == item.dot_position and 
                                cust.start_position == item.start_position):
                                self._customers[next_sym][i] = item
                                break
                
                # Update the index to point to the new item object
                # (This is important because the new item might have different backpointer)
                    if existing_idx < self._next:
                        self._items.append(item)
                        self._index[key] = len(self._items) - 1
    ...
    def get_customers(self, symbol: str) -> Iterable[Item]:
        """Get all items that have 'symbol' after the dot."""
        return self._customers.get(symbol, [])
...
class Item:
    ...
    weight: float = 0.0
    backpointer: Optional[Tuple] = None
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                best_weight = chart.best_weight()
                
                if best_weight is None:
                    # Sentence was rejected by grammar
                    print("NONE")
                else:
                    # Sentence was accepted - get and print parse tree
                    parse_tree = chart.get_best_parse()
                    print(parse_tree)
                    print(best_weight)  # Print weight on separate line, no f-string needed
