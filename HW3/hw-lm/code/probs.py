...
def my_read_trigrams(file: Path, vocab: Vocab) -> List[Trigram]:
    """Iterator over the trigrams in file.  Each triple (x,y,z) is a token z
    (possibly EOS) with a left context (x,y)."""
    x, y = BOS, BOS
    ret = []
...
def read_ngrams(file: Path, vocab: Vocab, n: int) -> List[List[Wordtype]]:
    cur_ngram = [BOS] * (n - 1)
    ngrams = []
    for z in read_tokens(file, vocab):
        cur_ngram.append(z)
        ngrams.append(cur_ngram.copy())
        del cur_ngram[0]
    return ngrams
...
class LanguageModel:
    ...
    # Sample using trigram probabilities the next token given the previous two tokens were x and y.
    def sample(self, x: Wordtype, y: Wordtype) -> Wordtype:
        log_probs = torch.zeros(self.vocab_size)
        ind_to_word = dict()
        for (i, word) in enumerate(self.vocab):
            log_probs[i] = self.log_prob(x, y, word)
            ind_to_word[i] = word

        # Draw from multinomial distribution described by log probabilities.
        dist = torch.distributions.Categorical(logits=log_probs)
        z = ind_to_word[dist.sample().item()]
        return z
...
class BackoffAddLambdaLanguageModel(AddLambdaLanguageModel):
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        ...
        pz = (self.event_count[z,] + self.lambda_) / (self.context_count[()] + self.lambda_ * self.vocab_size)
        pz_y = ((self.event_count[y, z] + self.lambda_ * self.vocab_size * pz) /
                    (self.context_count[y,] + self.lambda_ * self.vocab_size))
        return ((self.event_count[x, y, z] + self.lambda_ * self.vocab_size * pz_y) /
                    (self.context_count[x, y] + self.lambda_ * self.vocab_size))
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        # Integerizer maps words to column indices in embedding matrix.
        vocab_list = [token for token in self.vocab]
        self.integerizer = Integerizer(vocab_list)

        # List of embeddings that will be stacked into embedding matrix.
        embed_list = [None] * self.vocab_size
        self.OOL_embedding = None
        with open(lexicon_file) as f:
            ...
            for line in f:
                ...
                for i in range(1, len(vals)):
                    embedding[i - 1] = float(vals[i])

                if token == 'OOL':
                    self.OOL_embedding = embedding

                token_ind = self.integerizer.index(token)
                if token_ind is not None:
                    embed_list[token_ind] = embedding

        for i in range(self.vocab_size):
            if embed_list[i] is None:
                embed_list[i] = self.OOL_embedding

        # Matrix with all vocab word embeddings stacked is useful for fast computation of normalizing constant.
        # This is a (d x v) matrix, each word embedding is a column.
        self.embed_mat = torch.stack(embed_list).T
    ...
    def get_embedding(self, token: str) -> torch.Tensor:
        ...
        return self.embed_mat[:, token_ind]
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        next_token_logits = self.logits(x, y)
        z_ind = self.integerizer.index(z)
        if z_ind is None:
            z_ind = self.integerizer.index('OOL')
        p_xyz = next_token_logits[z_ind]
        log_norm_const = torch.logsumexp(next_token_logits, dim=0)
        return p_xyz - log_norm_const
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor,"vocab"]:
        ...
        embed_x = self.get_embedding(x)
        embed_y = self.get_embedding(y)
        xXz_vocab = embed_x.T @ self.X @ self.embed_mat
        yYz_vocab = embed_y.T @ self.Y @ self.embed_mat
        return xXz_vocab + yYz_vocab
    ...
    def train(self, file: Path):    # type: ignore
        ...
        eta0 = 0.00001  # initial learning rate
        ...
        for epoch in range(self.epochs):
            ...
            for (x, y, z) in read_trigrams(file, self.vocab):
                # Note that every matrix value in X and Y show up as feature coefficients, so we sum the
                # square of every matrix element in our regularizer term.
                neg_F_i = self.l2 / N * (self.X.square().sum() + self.Y.square().sum()) - self.log_prob_tensor(x, y, z)
                F -= neg_F_i.item()
                neg_F_i.backward()
                optimizer.step()
                optimizer.zero_grad()
            F /= N
            log.info(f"epoch {epoch + 1}: F = {F}")
...
class ImprovedLogLinearLanguageModel(EmbeddingLogLinearLanguageModel):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        # Batch size to accumulate gradients over
        self.batch_size = 32

        # Context window size used for computing embedding features.
        self.gram_size = 5

        # Decay factor to reduce influence of older tokens in the context.
        self.context_decay = 0.9
        decay_list = [math.pow(self.context_decay, self.gram_size - 3 - i) for i in range(self.gram_size - 2)]
        self.decay_tensor = torch.Tensor(decay_list)

        # Add feature for OOV.
        OOV_ind = self.integerizer.index('OOV')
        self.OOV_feature = torch.zeros(self.vocab_size)
        self.OOV_feature[OOV_ind] = 1.0
        self.OOV_param = nn.Parameter(torch.tensor(0.0), requires_grad=True)

        # Add parameter for unigram log-prob feature. Will be populated before training.
        self.unigram_log_prob_feature = torch.zeros(self.vocab_size)
        self.unigram_param = nn.Parameter(torch.tensor(0.0), requires_grad=True)
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor, "vocab"]:
        ...
        xXz_vocab = embed_x.T @ self.X @ self.embed_mat
        yYz_vocab = embed_y.T @ self.Y @ self.embed_mat
        return xXz_vocab + yYz_vocab + self.OOV_feature * self.OOV_param + self.unigram_log_prob_feature * self.unigram_param

    @typechecked
    def batch_log_prob_tensor(self, trigrams: List[Trigram]) -> TorchScalar:
        xs = [trigram[0] for trigram in trigrams]
        ys = [trigram[1] for trigram in trigrams]
        next_token_logits = self.batch_logits(xs, ys)

        log_prob = torch.tensor(0.0)
        for (i, trigram) in enumerate(trigrams):
            z_ind = self.integerizer.index(trigram[2])
            if z_ind is None:
                z_ind = self.integerizer.index('OOL')

            p_xyz = next_token_logits[i][z_ind]
            log_norm_const = torch.logsumexp(next_token_logits[i], dim=0)
            log_prob += p_xyz - log_norm_const
        return log_prob

    def batch_logits(self, xs: List[Wordtype], ys: List[Wordtype]) -> Float[torch.Tensor, "vocab"]:
        embed_xs = torch.stack([self.get_embedding(x) for x in xs])
        embed_ys = torch.stack([self.get_embedding(y) for y in ys])
        xXz_vocab = embed_xs @ self.X @ self.embed_mat
        yYz_vocab = embed_ys @ self.Y @ self.embed_mat
        return xXz_vocab + yYz_vocab + self.OOV_feature * self.OOV_param + self.unigram_log_prob_feature * self.unigram_param

    # Note: I did not end up using the three functions below which support arbitrary length ngrams, since they did
    # not appear to help classification.
    def ngram_log_prob(self, ngram: List[Wordtype]) -> float:
        return self.ngram_log_prob_tensor(ngram).item()

    @typechecked
    def ngram_log_prob_tensor(self, ngram: List[Wordtype]) -> TorchScalar:
        next_token_logits = self.ngram_logits(ngram[:-1])
        z_ind = self.integerizer.index(ngram[-1])
        if z_ind is None:
            z_ind = self.integerizer.index('OOL')
        p_ngram = next_token_logits[z_ind]
        log_norm_const = torch.logsumexp(next_token_logits, dim=0)
        return p_ngram - log_norm_const

    def ngram_logits(self, context: List[Wordtype]) -> Float[torch.Tensor, "vocab"]:
        embeddings = [self.get_embedding(context[i]) for i in range(len(context) - 1)]
        embeddings = torch.stack(embeddings)
        last_embedding = self.get_embedding(context[-1]).T

        # Start with non-embedding based features
        logits = self.OOV_feature * self.OOV_param + self.unigram_log_prob_feature * self.unigram_param

        # Add on embedding based features
        vals = (embeddings @ self.X @ self.embed_mat) * self.decay_tensor.unsqueeze(1)
        logits += vals.sum(dim=0) + last_embedding @ self.Y @ self.embed_mat
    ...
    def train(self, file: Path):  # type: ignore
        ...
        # Fill unigram counts
        for z in read_tokens(file, self.vocab):
            ind = self.integerizer.index(z)
            self.unigram_log_prob_feature[ind] += 1
        self.unigram_log_prob_feature = torch.log(self.unigram_log_prob_feature + 1)
        ...
        trigrams = my_read_trigrams(file, self.vocab)
        for epoch in range(self.epochs):
            F = 0
            np.random.shuffle(trigrams)
            batches = [trigrams[i:i + self.batch_size] for i in range(0, N, self.batch_size)]
            for batch in batches:
                # Note that every matrix value in X and Y show up as feature coefficients, so we sum the
                # square of every matrix element in our regularizer term.
                cur_size = len(batch)
                neg_F_i = self.l2 / N * (self.X.square().sum() + self.Y.square().sum()) * cur_size - self.batch_log_prob_tensor(batch)
                F -= neg_F_i.item()
                neg_F_i.backward()
                optimizer.step()
                optimizer.zero_grad()
